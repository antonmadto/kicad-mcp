...
## Current status
- Phase: **ALL PHASES 0–6 complete and green** (2026-07-04). Lint (ruff) + 227 pytest pass / 15 skip (GUI/live tests auto-skip headless; they PASSED against live KiCad 9.0.8). 45 tools + 4 router tools, 4 prompts, 23 review rules across 10 families. **Four adversarial multi-agent reviews run** (Phase 0/1, Phase 2, Phase 4–6, robustness audit 2026-07-18): 6 + 12 + 20 + 22 = 60 confirmed findings, ALL fixed.
- **Robustness audit round 4** (2026-07-18, branch `claude/kicad-mcp-multimodel-agents-mv4m1s`): multi-model sweep (7 finders + adversarial verifier per finding) confirmed 22 findings, all fixed with regression tests (59 tests added). Highlights: C4 no longer flags grounded guard traces as aggressors (anti-myth, one finding per pair stem); `(arc ...)` copper now sampled into the model (net lengths were understating F2/F4); `geometry.rotate` sign matched to KiCad's Y-down RotatePoint (rotated pads were reflected); G1 counts electrically-continuous islands, not raw pours; keepout rule-areas are voids for R5/plane roles; PHIL-USB-2 E→W (internal DP pull-ups); DEC-1 measures from IC pads; `refill_zones` bounded by our own deadline (kipy's blocking poll can spin forever if KiCad dies — would wedge the whole server); `get_board` re-probes sibling sockets; `execute_tool` no longer double-JSON-encodes bare-dict tools; `set_design_context` rejects non-positive/non-finite rise times; export_gerbers/drill report overwritten files (mtime diff); edit_schematic ERC gates are non-fatal (a committed write is never reported failed); create_project rolls back on gate failure; env timeouts validated (nan/inf/≤0 → defaults); TaskStore bounded (max_finished=100); waypoint/polygon inputs shape-validated. Back-side (B.Cu) pad transform reviewed and deliberately left unchanged — KiCad stores post-flip local coords; needs a KiCad-authored flipped-footprint board to verify further. Highlights fixed: path-traversal guard, UTF-8 encoding, span-based plane detection (L-shaped boards), DEC-1 connector FP, R5 abutting-zone tolerance; and Phase 4–6: diff-pair bare-P/N mis-pairing (VIN/VIP false pair + USB_DP/DM miss), atomic schematic write (temp+os.replace — a failed edit now leaves the original byte-identical), C4 cross-segment overlap accumulation, microstrip prop delay 5.8→6.1 ps/mm, TaskStore locked snapshots + atexit shutdown, sqlite URI escaping, router double-wrap unwrap.
